...
import {issueGenerationAuthorization} from '@/app/lib/byteplus/generation-authorization.server';
...
export async function POST(request: NextRequest, context: {params: Promise<{projectId: string}>}) {
  try {
    ...
    return NextResponse.json(await issueGenerationAuthorization(project, body.attemptId, body.expectedRequestHash));
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Generation authorization failed.';
