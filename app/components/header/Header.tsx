    ...
    return (
        <header className="sticky top-0 z-40 border-b border-white/[0.07] bg-[#08090d]/85 backdrop-blur-xl">
            <div className="mx-auto flex h-16 max-w-7xl items-center gap-3 px-5 sm:gap-8 sm:px-8">
                <Link href="/" className="group flex shrink-0 items-center gap-2 sm:gap-3" aria-label="함께봄 Ai영상제작소 홈">
                    ...
                    <span className="hidden text-[15px] font-black tracking-[-0.03em] text-white sm:inline">함께봄 Ai영상제작소</span>
                    ...
                    {links.map((link) => {
                        ...
                        return (
                            <Link
                                ...
                                className={`whitespace-nowrap rounded-lg px-2 py-2 text-sm font-semibold transition-colors sm:px-3 ${active ? 'bg-white/[0.08] text-white' : 'text-gray-400 hover:bg-white/[0.05] hover:text-white'}`}
                            >
                                {link.label}
                            </Link>
                        );
