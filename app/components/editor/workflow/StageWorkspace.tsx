    ...
    return (
      <section className="h-full overflow-y-auto bg-[#0c0b10] p-6 lg:p-10">
        <div className="mx-auto max-w-6xl">
          ...
          {!sampleMode ? <label className="mt-6 block rounded-2xl border border-white/10 bg-white/[0.035] p-4"><span className="text-sm font-black text-white">운영 승인 암호</span><span className="mt-1 block text-xs leading-5 text-gray-400">유료 이미지 생성 권한을 확인합니다. 이 값은 프로젝트에 저장되지 않으며 새로고침하면 지워집니다.</span><input type="password" autoComplete="off" value={imageApprovalToken} disabled={generationBusy} onChange={(event) => setImageApprovalToken(event.target.value)} placeholder="운영 승인 암호 입력" className="mt-3 w-full rounded-xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-white outline-none focus:border-fuchsia-400 disabled:opacity-50"/></label> : null}
