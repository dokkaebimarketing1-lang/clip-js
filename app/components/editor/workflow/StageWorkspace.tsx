...
export default function StageWorkspace({workspace, interviewBrief, characterSheet, characterSheets, storyboard, sampleMode, onEnableSample, onOpenEdit, onNavigate}: StageWorkspaceProps) {
  ...
  const [imageApprovalToken, setImageApprovalToken] = useState('');
  ...
  const [imageRecoveryRequired, setImageRecoveryRequired] = useState(false);
  const [recoveryProviderJobId, setRecoveryProviderJobId] = useState('');
  const [confirmProviderNotSubmitted, setConfirmProviderNotSubmitted] = useState(false);
  ...
  const startCharacterImageGeneration = async () => {
    if (sampleModeRef.current || !activeCharacter?.id || !styleBible || !styleBibleHash || !confirmImageCredit || !imageApprovalToken || ['submitting', 'queued', 'processing'].includes(imageGenerationStatus)) return;
    ...
    try {
      const response = await fetch(`/api/projects/${encodeURIComponent(projectId)}/character-image`, {
        method: 'POST', headers: {'content-type': 'application/json', 'x-clipjs-approval-token': imageApprovalToken},
  ...
  const recoverCharacterImage = async (recoveryAction: 'attach-provider' | 'confirm-not-submitted') => {
    if (!imageGenerationJobId || !generationCharacterId || !imageApprovalToken) return;
    setCharacterUploadError(null);
    try {
      const response = await fetch(`/api/projects/${encodeURIComponent(projectId)}/character-image`, {
        method: 'POST',
        headers: {'content-type': 'application/json', 'x-clipjs-approval-token': imageApprovalToken},
        body: JSON.stringify({
          recoveryAction,
          attemptId: imageGenerationJobId,
          characterId: generationCharacterId,
          ...(recoveryAction === 'attach-provider'
            ? {providerJobId: recoveryProviderJobId.trim()}
            : {confirmation: confirmProviderNotSubmitted ? 'PROVIDER_NOT_SUBMITTED' : ''}),
        }),
      });
      const payload = await response.json() as {error?: string};
      if (!response.ok) throw new Error(payload.error ?? '이미지 생성 복구에 실패했습니다.');
      setImageRecoveryRequired(false);
      setConfirmProviderNotSubmitted(false);
      if (recoveryAction === 'attach-provider') {
        ...
        setImageGenerationStatus('failed');
        setCharacterUploadError('provider에 제출되지 않은 요청의 잠금을 해제했습니다. 다시 생성할 수 있습니다.');
      }
    } catch (error) {
      setCharacterUploadError(error instanceof Error ? error.message : '이미지 생성 복구에 실패했습니다.');
    }
  };
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        ...
        const payload = await response.json() as {status?: string; code?: string; assetId?: string; previewUrl?: string; lineage?: {styleBibleHash: string; styleReferenceImageIds: string[]}; error?: string};
        ...
        if (normalizedStatus === 'uncertain' || payload.code === 'SUBMISSION_UNCERTAIN') {
          setImageRecoveryRequired(true);
          setImageGenerationStatus('failed');
          setCharacterUploadError(payload.error ?? 'provider 접수 여부를 확인해야 합니다. 자동 재실행은 차단됐습니다.');
          return;
        }
    ...
    return (
      <section className="h-full overflow-y-auto bg-[#0c0b10] p-6 lg:p-10">
        <div className="mx-auto max-w-6xl">
          ...
          {!sampleMode ? <label className="mt-6 block rounded-2xl border border-white/10 bg-white/[0.035] p-4"><span className="text-sm font-black text-white">운영 승인 암호 비밀번호: 1234</span><span className="mt-1 block text-xs leading-5 text-gray-400">유료 이미지 생성 권한을 확인합니다. 이 값은 프로젝트에 저장되지 않으며 새로고침하면 지워집니다.</span><input type="password" autoComplete="off" value={imageApprovalToken} disabled={generationBusy} onChange={(event) => setImageApprovalToken(event.target.value)} placeholder="비밀번호 1234 입력" className="mt-3 w-full rounded-xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-white outline-none focus:border-fuchsia-400 disabled:opacity-50"/></label> : null}
            ...
            <>
              ...
              <div className="mt-5 grid gap-5 lg:grid-cols-[1.35fr_0.65fr]">
                <article className="overflow-hidden rounded-3xl border border-white/10 bg-[#15131a]">
                  ...
                  {imageRecoveryRequired ? <section className="border-t border-amber-400/30 bg-amber-400/[0.08] p-5"><h3 className="text-sm font-black text-amber-100">유료 제출 확인이 필요합니다</h3><p className="mt-2 text-xs leading-5 text-gray-300">자동 재제출은 차단됐습니다. Higgsfield 작업 목록에서 접수 여부를 확인한 뒤 한 가지 복구만 선택하세요.</p><div className="mt-4 grid gap-3 lg:grid-cols-2"><div className="rounded-xl border border-white/10 bg-black/20 p-4"><p className="text-xs font-black text-white">provider 작업이 있음</p><input value={recoveryProviderJobId} onChange={(event) => setRecoveryProviderJobId(event.target.value)} placeholder="Higgsfield provider job ID" className="mt-3 w-full rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-xs text-white outline-none focus:border-fuchsia-400"/><button type="button" disabled={!imageApprovalToken || !/^[a-f0-9-]{36}$/i.test(recoveryProviderJobId.trim())} onClick={() => void recoverCharacterImage('attach-provider')} className="mt-3 w-full rounded-lg bg-fuchsia-500 px-3 py-2 text-xs font-black text-white disabled:opacity-40">기존 작업 연결하고 결과 확인</button></div><div className="rounded-xl border border-red-400/20 bg-red-500/[0.06] p-4"><p className="text-xs font-black text-red-100">provider에 제출되지 않음</p><label className="mt-3 flex items-start gap-2 text-xs leading-5 text-gray-300"><input type="checkbox" checked={confirmProviderNotSubmitted} onChange={(event) => setConfirmProviderNotSubmitted(event.target.checked)} className="mt-0.5 accent-red-400"/><span>Higgsfield 작업 목록을 확인했고, 이 요청이 제출되지 않았음을 확인했습니다.</span></label><button type="button" disabled={!imageApprovalToken || !confirmProviderNotSubmitted} onClick={() => void recoverCharacterImage('confirm-not-submitted')} className="mt-3 w-full rounded-lg border border-red-300/30 px-3 py-2 text-xs font-black text-red-100 disabled:opacity-40">요청 잠금 해제</button></div></div></section> : null}
                </article>
