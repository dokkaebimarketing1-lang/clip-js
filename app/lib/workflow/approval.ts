...
import {storyboardSchema, type Storyboard, type StoryboardApproval, type WorkflowState} from './schema';
...
export const assertVideoGenerationAllowed = async (workflow: WorkflowState): Promise<void> => {
  if (!workflow.storyboard || workflow.approval.status !== 'approved' || !workflow.approval.storyboardHash) {
