  ...
  return (
    <main className="relative overflow-hidden">
      ...
      <section className="relative mx-auto max-w-5xl px-5 pb-16 pt-24 text-center sm:px-8 sm:pt-32">
        ...
        <h1 className="mx-auto mt-5 max-w-4xl break-keep text-balance text-4xl font-black leading-tight tracking-[-0.04em] text-white sm:text-5xl">사람이 결정할 수 있는<br className="hidden sm:block" />{' '}AI 영상 제작 도구를 만듭니다.</h1>
        <p className="mx-auto mt-7 max-w-2xl break-keep text-pretty text-base leading-8 text-gray-400 sm:text-lg">함께봄 Ai영상제작소는 함께봄 주식회사가 만드는 AI 영상 제작 스튜디오입니다. 빠른 생성보다 제작자가 결과를 이해하고, 비교하고, 자신의 손으로 확정할 수 있는 흐름을 우선합니다.</p>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-8 lg:px-10">
        <div className="grid gap-4 lg:grid-cols-3">
          {values.map(({icon: Icon, title, body}, index) => <article key={title} className="clip-panel p-7"><div className="flex items-center justify-between"><span className="grid h-12 w-12 place-items-center rounded-2xl border border-fuchsia-400/20 bg-fuchsia-400/[0.07] text-fuchsia-300"><Icon size={20} aria-hidden="true" /></span><span className="text-[10px] font-black tracking-[0.2em] text-gray-700">0{index + 1}</span></div><h2 className="mt-7 break-keep text-xl font-black text-white">{title}</h2><p className="mt-4 break-keep text-pretty text-sm leading-7 text-gray-500">{body}</p></article>)}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 pb-24 sm:px-8 lg:px-10">
        <div className="overflow-hidden rounded-[28px] border border-white/[0.08] bg-gradient-to-br from-[#17121c] to-[#101119] p-8 sm:p-12">
          <div className="grid gap-10 lg:grid-cols-[1fr_.9fr] lg:items-center">
            <div><p className="text-[11px] font-black uppercase tracking-[0.22em] text-violet-300">Production protocol</p><h2 className="mt-4 break-keep text-2xl font-black tracking-tight text-white sm:text-3xl">결정 → 실행 → 검수의 경계를 분명하게</h2><p className="mt-5 max-w-xl break-keep text-pretty text-sm leading-7 text-gray-400">현재 대상과 결과를 확인하고, 비용과 변경 영향을 이해한 뒤 명시적으로 실행합니다. 비동기 작업은 저장 상태를 보여 주고, 도착한 결과는 승인·반려·재시도 과정을 거쳐 다음 단계로 이동합니다.</p></div>
